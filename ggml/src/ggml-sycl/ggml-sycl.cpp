...
GGML_BACKEND_API void ggml_backend_sycl_get_gpu_list(int *id_list, int max_len) try {
    ...
    for (int i=0;i< ggml_sycl_info().device_count;i++){
        if (i>=max_len) break;
        id_list[i] = ggml_sycl_info().ids[i];
    }
    return;
}
    ...
    std::vector<ggml_tensor_extra_gpu *> tensor_extras;
     ...
     ggml_backend_sycl_buffer_context(int device_id, void * dev_ptr, queue_ptr stream) :
        device(device_id), dev_ptr(dev_ptr), stream(stream) {
            ...
            opt_feature = ggml_sycl_info().infos[device].opt_feature;
        }
    ...
    if (!ggml_backend_sycl_buffer_type_initialized) {
        for (auto & id: ggml_sycl_info().ids) {
            ...
            ggml_backend_sycl_buffer_types[id] = {
                ...
                /* .device   = */ ggml_backend_reg_dev_get(ggml_backend_sycl_reg(), id),
...
struct ggml_sycl_pool_leg : public ggml_sycl_pool {
    ...
    explicit ggml_sycl_pool_leg(queue_ptr qptr_, int device_) : device_id(device_), qptr(qptr_) {}
...
void ggml_sycl_set_main_device(const int main_device_id) try {
    if (dpct::get_current_device_id() == static_cast<unsigned int> (main_device_id)) return;
...
void ggml_backend_sycl_get_device_memory(int device_id, size_t *free,
                                                   size_t *total) try {
...

    {
        ...
        if (!initialized) {
            ...
            for (auto & id: ggml_sycl_info().ids) {
                ...
                dev_ctx->device = id;
                dev_ctx->name = GGML_SYCL_NAME + std::to_string(id);
...
ggml_backend_t ggml_backend_sycl_init(int device_id) {
    ...
    ggml_backend_t sycl_backend = new ggml_backend {
        ...
        /* .device    = */ ggml_backend_reg_dev_get(ggml_backend_sycl_reg(), device_id),
        /* .context   = */ ctx
    };
